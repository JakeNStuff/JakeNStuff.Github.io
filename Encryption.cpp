...
std::string read_file(const std::string& filename)
{
  ...
  //first open the file_text
  std::ifstream ifile(filename);
  //read the file entirely into the string
...
void save_data_file(const std::string& file_name, const std::string& student_name, const std::string& key, const std::string& data)
{
	std::ofstream file(file_name);
	time_t timestamp = time(NULL);
	//string timestamp="2021-04-05";
	if (file.is_open())
	{
